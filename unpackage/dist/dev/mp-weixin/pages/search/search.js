...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    common_vendor.onUnload(() => {
      common_vendor.index.removeStorageSync("storageClassList", classList.value);
    });
    const onSearch = () => {
      common_vendor.index.showLoading();
      historySearch.value = [.../* @__PURE__ */ new Set([params.value.keyword, ...historySearch.value])].slice(0, 10);
    ...
    const searchData = async () => {
      try {
        let res = await api_search.getSearchDataService(params.value);
    ...
    return (_ctx, _cache) => {
      return common_vendor.e({
        ...
        d: common_vendor.o(($event) => params.value.keyword = $event),
        e: common_vendor.p({
          focus: true,
          placeholder: "搜索",
          modelValue: params.value.keyword
        }),
